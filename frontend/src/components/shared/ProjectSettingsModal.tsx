...
const projectSettingsI18n = {
  ...
  ru: {
    projectSettings: {
      title: "Настройки", projectConfig: "Настройки проекта", exportConfig: "Настройки экспорта", globalConfig: "Глобальные настройки",
      projectConfigTitle: "Конфигурация проекта", projectConfigDesc: "Эти настройки применяются только к текущему проекту и не влияют на другие проекты",
      globalConfigTitle: "Глобальные настройки", globalConfigDesc: "Эти настройки применяются ко всем проектам",
      aspectRatio: "Соотношение сторон", aspectRatioDesc: "Задать соотношение сторон для генерируемых изображений слайдов",
      aspectRatioLocked: "Нельзя изменить соотношение сторон после генерации изображений",
      aspectRatioHelp: "Некоторые модели поддерживают только определённые соотношения сторон (например, 16:9, 4:3, 1:1). Если при генерации изображений возникает ошибка, попробуйте выбрать другое соотношение сторон.",
      extraRequirements: "Дополнительные требования", extraRequirementsDesc: "AI будет учитывать эти дополнительные требования при создании каждого слайда",
      extraRequirementsPlaceholder: "Например: использовать компактную компоновку, разместить вверху заголовок пункта плана первого уровня, добавить более разнообразные иллюстрации PPT...",
      saveExtraRequirements: "Сохранить дополнительные требования",
      styleDescription: "Описание стиля", styleDescriptionDesc: "Опишите желаемый общий стиль PPT, и AI создаст страницы в этом стиле",
      styleDescriptionPlaceholder: "Например: сдержанный деловой стиль, тёмно-синяя и белая цветовая гамма, чёткие шрифты, аккуратная компоновка...",
      saveStyleDescription: "Сохранить описание стиля",
      styleTip: "Описание стиля будет автоматически добавляться в промпт при генерации изображений. Если также загружено изображение шаблона, описание стиля будет использоваться в качестве дополнительных указаний.",
      editablePptxExport: "Настройки экспорта редактируемого PPTX", editablePptxExportDesc: "Настройте работу функции «Экспортировать редактируемый PPTX». Эти параметры влияют на качество экспорта и стоимость вызовов API.",
      extractorMethod: "Метод извлечения компонентов", extractorMethodDesc: "Выберите способ извлечения редактируемых компонентов, таких как текст и таблицы, из изображений PPT",
      extractorHybrid: "Гибридное извлечение (рекомендуется)", extractorHybridDesc: "Анализ макета MinerU + высокоточный OCR Baidu для более точного распознавания текста",
      extractorMineru: "Извлечение с помощью MinerU", extractorMineruDesc: "Использовать только MinerU для анализа макета и распознавания текста",
      backgroundMethod: "Способ получения фонового изображения", backgroundMethodDesc: "Выберите способ создания чистого фонового изображения: удаление текста с исходного изображения для использования в качестве фона PPT",
      backgroundHybrid: "Гибридный способ (рекомендуется)", backgroundHybridDesc: "Точное удаление текста Baidu + повышение качества с помощью генеративной модели",
      backgroundGenerative: "Генеративный способ", backgroundGenerativeDesc: "Использовать генеративную модель (например, Gemini) для непосредственного создания фона: качество будет высоким, но могут сохраниться отдельные элементы исходного изображения",
      backgroundBaidu: "Восстановление с помощью Baidu", backgroundBaiduDesc: "Использовать API восстановления изображений Baidu: работает быстро, но качество среднее",
      usesAiModel: "Использует ИИ-модель для генерации изображений",
      costTip: "Варианты, помеченные как «Использует ИИ-модель для генерации изображений», вызывают API генерации изображений на основе ИИ (например, Gemini), что приводит к дополнительным расходам на API для каждой страницы. Для контроля затрат выберите «Восстановление с помощью Baidu».",
      iconSubjectExtraction: "Прозрачный фон для значков",
      iconSubjectExtractionDesc: "При включении этой функции изображения, распознанные при экспорте как значки, обрабатываются локальной моделью RMBG-2.0 для удаления фона и получения PNG с прозрачным фоном, после чего вставляются в PPT. Это предотвращает заметное отображение исходного фона PPT значка на новом фоне слайда. Применяется только к элементам, распознанным как значки; фотографии и иллюстрации сохраняют исходную прямоугольную обрезку.",
      iconSubjectExtractionTip: "Обработка выполняется локально с использованием модели RMBG-2.0 ONNX. При первом использовании модель размером около 512MB будет загружена автоматически. Если обработка отдельного значка завершится ошибкой, будет автоматически использована исходная прямоугольная обрезка; это не повлияет на экспорт.",
      errorHandling: "Стратегия обработки ошибок", errorHandlingDesc: "Настройте обработку ошибок, возникающих во время экспорта",
      allowPartialResult: "Разрешить частичный результат", allowPartialResultDesc: "Если параметр включён, при возникновении ошибок во время экспорта (например, при сбое извлечения стилей или отрисовки текста) ошибки будут пропущены, а экспорт продолжится; в результате файл может оказаться неполным. При отключении любая ошибка немедленно остановит экспорт с указанием конкретной причины.",
      allowPartialResultWarning: "Включение этого параметра может привести к утрате части стилей текста, неверному размещению элементов или отсутствию содержимого в экспортированном файле PPTX. Рекомендуется включать его только при необходимости быстро получить результат и готовности принять снижение качества.",
      saveExportSettings: "Сохранить настройки экспорта",
      tip: "Совет"
    },
    shared: { saving: "Сохранение..." }
  },
};
